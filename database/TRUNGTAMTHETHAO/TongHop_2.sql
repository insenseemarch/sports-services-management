...
CREATE DATABASE TRUNGTAMTHETHAO_FIXED
GO
...
-- [T1] Kiểm tra tình trạng sân (FIX: REPEATABLE READ)
...
BEGIN
    SET TRAN ISOLATION LEVEL REPEATABLE READ 
    BEGIN TRAN
        PRINT N'T1: Kiểm tra lần 1...'
        SELECT TinhTrang FROM SAN WHERE MaSan = @MaSan;
        
        WAITFOR DELAY '00:00:10'; 
        
        PRINT N'T1: Kiểm tra lần 2 (Trạng thái phải KHÔNG đổi)...'
        SELECT TinhTrang FROM SAN WHERE MaSan = @MaSan;
    COMMIT TRAN
END
GO

-- [T2] Đặt sân Online (Input chuẩn báo cáo)
CREATE OR ALTER PROCEDURE sp_DatOnlineSan
    @MaDatSan BIGINT OUTPUT, 
    @MaKH VARCHAR(20),
    @NguoiLap VARCHAR(20),
    @NgayDat DATE,
    @NgayKetThuc DATE,
...
BEGIN
    SET TRAN ISOLATION LEVEL SERIALIZABLE 
    BEGIN TRAN
        PRINT N'T1: Đếm số sân trống lần 1...'
        SELECT * FROM SAN WHERE MaLS = @MaLS AND TinhTrang = N'Còn Trống';
        
        WAITFOR DELAY '00:00:10';
        
        PRINT N'T1: Đếm số sân trống lần 2 (Kết quả phải KHÔNG đổi)...'
        SELECT * FROM SAN WHERE MaLS = @MaLS AND TinhTrang = N'Còn Trống';
    COMMIT TRAN
END
GO
...
BEGIN
    BEGIN TRAN
        PRINT N'T2: Đang cố thêm sân trống...'
        -- Update S03 (đang bảo trì) thành Còn Trống
        UPDATE SAN SET TinhTrang = N'Còn Trống' WHERE MaSan = @MaSanMoi AND MaLS = @MaLS;
        PRINT N'T2: Đã thêm sân trống thành công.'
    COMMIT TRAN
END
GO
...
-- [T1] Quản lý A cập nhật (FIX: REPEATABLE READ -> Deadlock)
CREATE OR ALTER PROCEDURE sp_CapNhatTyLeGiamGiaA
    @MaUD VARCHAR(20),
    @TilegiamgiaA DECIMAL(5,2)
AS
BEGIN
    SET TRAN ISOLATION LEVEL REPEATABLE READ; 
    BEGIN TRY
        BEGIN TRAN
            ...
            SELECT @TyLeCu = TyLeGiamGia FROM UUDAI WHERE MaUD = @MaUD; -- Giữ khóa S
            PRINT N'T1: Đã đọc tỷ lệ cũ. Đang tính toán...'
            
            WAITFOR DELAY '00:00:10';

            UPDATE UUDAI SET TyLeGiamGia = @TilegiamgiaA WHERE MaUD = @MaUD; -- Cần khóa X
        COMMIT TRAN 
        PRINT N'T1: Cập nhật thành công.'
    END TRY
    BEGIN CATCH
        ROLLBACK TRAN;
        PRINT N'T1 Lỗi: Bị Deadlock (Hệ thống đã chặn Lost Update).';
    END CATCH
END
GO

-- [T2] Quản lý B cập nhật
CREATE OR ALTER PROCEDURE sp_CapNhatTyLeGiamGiaB
    @MaUD VARCHAR(20),
    @TilegiamgiaB DECIMAL(5,2)
AS
BEGIN
    SET TRAN ISOLATION LEVEL REPEATABLE READ;
    BEGIN TRY
        BEGIN TRAN
            ...
            PRINT N'T2: Đã đọc tỷ lệ cũ. Đang tính toán...'
            
            WAITFOR DELAY '00:00:10';

            UPDATE UUDAI SET TyLeGiamGia = @TilegiamgiaB WHERE MaUD = @MaUD;
        COMMIT TRAN
        PRINT N'T2: Cập nhật thành công.'
    END TRY
    BEGIN CATCH
        ROLLBACK TRAN;
        PRINT N'T2 Lỗi: Bị Deadlock (Hệ thống đã chặn Lost Update).';
    END CATCH
END
GO
...
-- [T1] Thanh toán Online (Insert rác rồi Rollback)
...
BEGIN
    BEGIN TRAN
        -- 1. Insert rác (Chưa commit)
        INSERT INTO CT_DICHVUDAT (MaDV, MaDatSan, SoLuong, SoGioThue, ThanhTien, TrangThaiSuDung, GhiChu)
        VALUES (@MaDV, @MaDatSan, @SoLuong, @SoGioThue, @ThanhTien, N'Đã thanh toán', N'Online');
        
        PRINT N'T1: Đã thêm dịch vụ (Chưa Commit). Chờ 10s...'
        WAITFOR DELAY '00:00:10';

        -- 2. Insert lỗi để Rollback
        BEGIN TRY
             INSERT INTO HOADON (MaPhieu, NgayLap, TongTien) VALUES (@Maphieuloi, GETDATE(), @ThanhTien);
        END TRY
        BEGIN CATCH
             PRINT N'T1: Lỗi Insert Hóa đơn -> Rollback toàn bộ.';
             ROLLBACK TRAN;
             RETURN;
        END CATCH
...
-- [T2] Xuất hóa đơn (FIX: READ COMMITTED)
...
AS
BEGIN
    SET TRAN ISOLATION LEVEL READ COMMITTED -- Chặn đọc bẩn
    BEGIN TRAN
        PRINT N'T2: Đang chờ dữ liệu sạch...'
        
        -- Sẽ bị Block cho đến khi T1 xong
        SELECT * FROM CT_DICHVUDAT 
        WHERE MaDatSan = @MaDatSan AND TrangThaiSuDung = N'Đã thanh toán';

        PRINT N'T2: Đọc xong. Không tìm thấy dữ liệu rác (hoặc đã an toàn).';
        
        INSERT INTO HOADON (MaPhieu, NguoiLap, NgayLap, TongTien, GiamGia, ThanhTien, HinhThucTT)
        VALUES (@MaDatSan, @MaNV, @NgayLap, @TongTien, @GiamGia, @ThanhTien, @Hinhthuctt);
    COMMIT TRAN
END
GO
-- Tình huống 11: Unrepeatable Read (Lập HĐ xem giá)
-- [T1]: Lập hóa đơn (FIX: REPEATABLE READ)
CREATE OR ALTER PROCEDURE sp_LapHoaDon
    @MaDatSan BIGINT, -- Input giữ nguyên theo báo cáo
    @MaLS VARCHAR(20)
AS
BEGIN
    -- GIẢI PHÁP: REPEATABLE READ giữ khóa S, T2 không thể sửa giá
    SET TRAN ISOLATION LEVEL REPEATABLE READ 
    BEGIN TRAN
        PRINT N'T1: --- BẮT ĐẦU ĐỌC GIÁ LẦN 1 ---'
        DECLARE @Gia1 DECIMAL(18,2);
        
        -- Lấy giá và hiển thị ra bảng
        SELECT @Gia1 = GiaApDung FROM KHUNGGIO WHERE MaLS = @MaLS;
        SELECT @Gia1 AS [Giá Lần 1 (Trước khi chờ)];

        PRINT N'T1: Đang chờ 10s... (T2 hãy thử đổi giá lúc này)'
        WAITFOR DELAY '00:00:10';

        PRINT N'T1: --- BẮT ĐẦU ĐỌC GIÁ LẦN 2 ---'
        DECLARE @Gia2 DECIMAL(18,2);
        
        -- Lấy lại giá và hiển thị
        SELECT @Gia2 = GiaApDung FROM KHUNGGIO WHERE MaLS = @MaLS;
        SELECT @Gia2 AS [Giá Lần 2 (Sau khi chờ)];
        
        -- So sánh và kết luận
        IF @Gia1 = @Gia2 
            PRINT N'T1: => KẾT LUẬN: Giá ổn định (Đã xử lý thành công).';
        ELSE 
            PRINT N'T1: => LỖI: Giá bị thay đổi giữa 2 lần đọc!';
    COMMIT TRAN
END
GO

-- [T2] Thay đổi giá
CREATE OR ALTER PROCEDURE sp_Thaydoigia
    @MaLS VARCHAR(20), @GiaMoi DECIMAL(18,2)
AS
BEGIN
    BEGIN TRAN
        PRINT N'T2: Đang chờ để đổi giá...'
        UPDATE KHUNGGIO SET GiaApDung = @GiaMoi WHERE MaLS = @MaLS;
        PRINT N'T2: Đổi giá thành công.'
    COMMIT TRAN
END
GO

-- Tình huống 12: Phantom Read (Báo cáo số lượng vs Đặt sân mới)
-- [T1] Lập báo cáo (FIX: SERIALIZABLE)
CREATE OR ALTER PROCEDURE sp_LapBaoCao
    @Ngaylap DATE
AS
BEGIN
    -- GIẢI PHÁP: SERIALIZABLE khóa phạm vi, chặn Insert mới
    SET TRAN ISOLATION LEVEL SERIALIZABLE 
    BEGIN TRAN
        PRINT N'T1: --- ĐẾM SỐ LƯỢNG LẦN 1 ---'
        SELECT COUNT(*) AS SoLuongPhieu FROM PHIEUDATSAN WHERE NgayDat = @Ngaylap AND KenhDat = N'Online';
        
        PRINT N'T1: Đang chờ 10s...'
        WAITFOR DELAY '00:00:10';
        
        PRINT N'T1: --- ĐẾM SỐ LƯỢNG LẦN 2 ---'
        SELECT COUNT(*) AS SoLuongPhieu FROM PHIEUDATSAN WHERE NgayDat = @Ngaylap AND KenhDat = N'Online';
    COMMIT TRAN
END
GO

-- [T2] Hủy Sân (Insert phiếu mới)
CREATE OR ALTER PROCEDURE sp_HuySan
    @MaSan VARCHAR(20), @MaDV VARCHAR(20), @MaKH VARCHAR(20), @MaNV VARCHAR(20), @Ngaylap DATE, @NgayDat DATE, @NgayKetThuc DATE, @GioBatDau TIME, @GioKetThuc TIME
AS
BEGIN
    BEGIN TRAN
        PRINT N'T2: Đang chờ để thêm phiếu mới...'
        INSERT INTO PHIEUDATSAN (MaKH, NguoiLap, NgayDat, NgayKetThuc, GioBatDau, GioKetThuc, KenhDat, TrangThai)
        VALUES (@MaKH, @MaNV, @NgayDat, @NgayKetThuc, @GioBatDau, @GioKetThuc, N'Online', N'Đã đặt');
        
        DECLARE @MaDatSan BIGINT = SCOPE_IDENTITY();
        INSERT INTO DATSAN (MaDatSan, MaSan) VALUES (@MaDatSan, @MaSan);
        
        UPDATE SAN SET TinhTrang = N'Đã đặt' WHERE MaSan = @MaSan;
        PRINT N'T2: Đã thêm phiếu mới thành công.'
    COMMIT TRAN
END
GO
...
CREATE OR ALTER PROCEDURE sp_Demo_XemGiaSan_RepeatableRead
    @MaLS VARCHAR(20),
    @GioBatDau TIME,
    @NgayDat DATE
AS
BEGIN
    ...
    BEGIN TRAN;
        ...
        PRINT N'🔹 Lần đọc giá thứ 1';

        SELECT TOP 1 GiaApDung AS Gia_Lan_1
        FROM KHUNGGIO
        WHERE MaLS = @MaLS
        ...
        PRINT N'⏳ Đang chờ 15s – hãy thử UPDATE giá ở session khác';
        WAITFOR DELAY '00:00:15';

        PRINT N'🔹 Lần đọc giá thứ 2';

        SELECT TOP 1 GiaApDung AS Gia_Lan_2
        FROM KHUNGGIO
        WHERE MaLS = @MaLS
          AND @GioBatDau >= GioBatDau
          AND @GioBatDau < GioKetThuc
...
CREATE OR ALTER PROCEDURE sp_Demo_UpdateGiaSan
    @MaLS VARCHAR(20),
    @MaKG VARCHAR(20),
    @GiaMoi DECIMAL(18,2)
AS
BEGIN
    BEGIN TRAN;
        PRINT N'⚠️ Đang cố UPDATE giá...';
        UPDATE KHUNGGIO
        SET GiaApDung = @GiaMoi
        WHERE MaLS = @MaLS AND MaKG = @MaKG;
    COMMIT TRAN;
END
GO
