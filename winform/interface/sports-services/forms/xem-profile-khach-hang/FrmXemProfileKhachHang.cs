...
{
    public partial class FrmXemProfileKhachHang : Form
    {
        public FrmXemProfileKhachHang()
        {
            InitializeComponent(); this.WindowState = FormWindowState.Maximized;
        }

        // Giả sử đây là ID của khách hàng đang đăng nhập
        private string currentMaKH = "KH001";

        private void Form1_Load(object sender, EventArgs e)
        {
            checkHSSV.AutoCheck = false;
            checkNL.AutoCheck = false;
            checkNCT.AutoCheck = false;

            LoadThongTinKhachHang(currentMaKH);
        }
        ...
        private void LoadThongTinKhachHang(string maKH)
        {
            ...
            // Bảng CẤP BẬC (Join với Khách hàng)
            string tenCapBac = "Thành viên Vàng";
            string uuDai = "Giảm 10% phí thuê sân";
            ...
            // Nhóm tài khoản
            txtPass.Text = "123";
            txtMaCapBac.Text = "001";
            txtTenDangNhap.Text = tenDangNhap;
            dtpNgayDangKy.Value = ngayDangKy;
            txtMaCapBac.Text = "MaVang";
            txtCapBac.Text = tenCapBac;
            txtUuDai.Text = uuDai;
        ...
        private void dtpNgaySinh_ValueChanged(object sender, EventArgs e)
        {
            ...
            // 2. Reset lại các checkbox trước khi chọn
            checkHSSV.Checked = false;
            checkNL.Checked = false;
            checkNCT.Checked = false;

            // 3. Ràng buộc logic (Bạn có thể sửa số tuổi ở đây)
            if (tuoi <= 22)
            {
                checkHSSV.Checked = true;
            }
            else if (tuoi > 22 && tuoi <= 60)
            {
                checkNL.Checked = true;
            }
            else // Trường hợp >= 60
            {
                checkNCT.Checked = true;
            }

            // (Tùy chọn) Hiển thị tuổi để debug xem đúng không
            // MessageBox.Show("Tuổi tính được là: " + tuoi);
        }
