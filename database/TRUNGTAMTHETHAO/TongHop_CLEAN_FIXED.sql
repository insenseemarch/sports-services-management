...
-- 3.1. BẢNG ĐÁNH GIÁ (Update từ update_schema_actions.sql)
CREATE TABLE DANHGIA (
    ...
    -- FK sẽ được check sau hoặc add ở cuối file constraint nếu có, ở đây add inline luôn nếu bảng PHIEUDATSAN đã có
...
CREATE TABLE KHUNGGIO (
    ...
    LoaiNgay NVARCHAR(50) DEFAULT N'Thường', -- Thường, Cuối tuần, Lễ
...
-- 26. BẢNG THAM SỐ HỆ THỐNG (NEW)
CREATE TABLE THAMSO_HETHONG(
    MaThamSo VARCHAR(50) NOT NULL PRIMARY KEY,
    TenThamSo NVARCHAR(255) NULL,
    GiaTri NVARCHAR(255) NULL,
    DonVi NVARCHAR(50) NULL, -- Đơn vị: %, giờ, phút, VNĐ, etc.
    MoTa NVARCHAR(500) NULL,
    LoaiThamSo NVARCHAR(50) NULL, -- 'Tiền tệ', 'Số', 'Văn bản', 'Thời gian'
    NgayCapNhat DATETIME DEFAULT GETDATE()
)
GO


-- 27. BẢNG ƯU ĐÃI - ÁP DỤNG (UUDAI - HOADON)
...
-- =============================================
-- PATCH: Allow Maintenance Staff to Update Court Status
-- Description: Modify trg_ChuyenTrangThaiSan to allow status updates
--              when triggered by PHIEUBAOTRI updates
-- =============================================
...
IF OBJECT_ID('trg_ChuyenTrangThaiSan', 'TR') IS NOT NULL
    DROP TRIGGER trg_ChuyenTrangThaiSan;
GO

-- Recreate trigger with maintenance exception
CREATE TRIGGER trg_ChuyenTrangThaiSan
ON SAN
AFTER UPDATE
AS
BEGIN
    ...
    -- Check if this update is from a maintenance operation
    -- We detect this by checking if CONTEXT_INFO is set
    DECLARE @ContextInfo VARBINARY(128);
    SET @ContextInfo = CONTEXT_INFO();
    
    -- If CONTEXT_INFO = 0x4D41494E54454E414E4345 (hex for 'MAINTENANCE')
    -- then skip validation
    IF @ContextInfo = 0x4D41494E54454E414E4345
    BEGIN
        RETURN;
    END
    
    -- Original validation logic for non-maintenance updates
    IF UPDATE(TinhTrang)
    BEGIN
        DECLARE @OldStatus NVARCHAR(50), @NewStatus NVARCHAR(50), @MaSan CHAR(10);
        
        SELECT @MaSan = i.MaSan, @OldStatus = d.TinhTrang, @NewStatus = i.TinhTrang
        FROM inserted i
        INNER JOIN deleted d ON i.MaSan = d.MaSan;
        ...
        -- Any -> Không Hoạt Động
        -- Không Hoạt Động -> Còn Trống
        
        IF NOT (
            (@OldStatus = N'Còn Trống' AND @NewStatus = N'Đã đặt') OR
            (@OldStatus = N'Đã đặt' AND @NewStatus = N'Còn Trống') OR
            (@OldStatus = N'Còn Trống' AND @NewStatus = N'Đang Bảo Trì') OR
            (@OldStatus = N'Đang Bảo Trì' AND @NewStatus = N'Còn Trống') OR
            (@NewStatus = N'Không Hoạt Động') OR
            (@OldStatus = N'Không Hoạt Động' AND @NewStatus = N'Còn Trống')
        )
        BEGIN
            RAISERROR(N'Lỗi: Chuyển đổi trạng thái sân không hợp lệ!', 16, 1);
            ROLLBACK TRANSACTION;
...
PRINT 'Trigger trg_ChuyenTrangThaiSan has been updated to allow maintenance status updates';
GO
USE master
GO
